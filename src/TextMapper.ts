...
export default class TextMapper {
    constructor() {
        console.log('TextMapper initialized');
        this.init();
    }
    ...
    async createMap(input: TextMapperInput, initialOptions: Partial<TextMapperOptions> = defaultOptions) {
        ...
        // Clone the document
        let domTree: HTMLElement = document.documentElement.cloneNode(true) as HTMLElement;
    ...
    async renderMap(domTree: HTMLElement) {
        ...
        mapWindow.document.write('<!DOCTYPE html><html><head><title>Text Map</title></head><body>');
